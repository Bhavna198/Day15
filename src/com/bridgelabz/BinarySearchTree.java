...
public class BinarySearchTree<K extends Comparable<K>> {
    ...
    private MyFirstNode<K> root;

    private MyFirstNode<K> addRecursively(MyFirstNode<K> current, K key) {

        if (current == null) {
            return new MyFirstNode<>(key);
        }
    ...
    private boolean searchTreeRecursively(MyFirstNode<K> current, K key) {
        if (current == null) {
            return false;
        }
        int compareResult = key.compareTo(current.key);
    ...
    private int getSizeRecursively(MyFirstNode<K> current) {
